...
public class PendingBookingExpirationWorker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "Worker xử lý booking và hàng chờ hết hạn đã gặp lỗi.");
            }
    ...
    private async Task ProcessExpiredBookingsAsync(CancellationToken stoppingToken)
    {
        ...
        var expiredBookings = await context.Bookings
            ...
            .Where(b => b.Status == BookingStatus.PendingPayment && b.PaymentDeadline < now)
        ...
        foreach (var booking in expiredBookings)
        {
            ...
            var waitlistEntry = await context.WaitlistEntries
                .FirstOrDefaultAsync(w => w.BookingId == booking.BookingId && w.Status == WaitlistStatus.Offered, stoppingToken);
            if (waitlistEntry != null)
            {
                waitlistEntry.Status = WaitlistStatus.Expired;
            }

            _logger.LogInformation("Đã hủy booking {BookingId} vì quá hạn thanh toán; hàng chờ sẽ chuyển sang người kế tiếp.", booking.BookingId);
        }
    ...
    private async Task ProcessExpiredWaitlistsAsync(CancellationToken stoppingToken)
    {
        ...
        var potentialExpiredWaitlists = await context.WaitlistEntries
            ...
            .Where(w => w.Status == WaitlistStatus.Waiting && w.PoolSlot.SlotDate <= currentDate)
            .ToListAsync(stoppingToken);
